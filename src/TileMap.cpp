		...
		for(i=0; i<=24; i++){
			for(j=0;j<=24; j++){
				...
				if(*(index) != (-1)){
					...
					this->tileSet->RenderTile(*(index),(j*tileWidth) + cameraX,(i*tileHeight) + CameraY+175);
          if(this->createCollision) CreateCollider(*(index),(j*tileWidth) + cameraX - 390, (i*tileHeight) + CameraY -300);
        }
			}
		}
	}/*else{
